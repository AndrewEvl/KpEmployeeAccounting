...
import com.evl.employeeaccounting.managers.department.IDepartmentManager;
...
public class DepartmentController {
    ...
    private final IDepartmentManager iDepartmentManager;

    @GetMapping("get-all")
    public ResponseEntity<?> getAll() {
        return ResponseEntity.ok()
                .body(iDepartmentManager.findAllDto());
    }

    @PostMapping("save")
    public ResponseEntity<?> save(@RequestBody NameDto rq) {
        Department department = iDepartmentManager.save(rq.getName());
        if (department != null) {
    ...
    public ResponseEntity<?> update(@RequestBody BaseData rq) {
        Department department = iDepartmentManager.update(rq.getId(), rq.getName());
        if (department != null) {
            return ResponseEntity.status(HttpStatus.CREATED).build();
        } else {
            return ResponseEntity.status(HttpStatus.I_AM_A_TEAPOT).build();
    ...
    public ResponseEntity<?> delete(@PathVariable("id") Long id) {
        try {
            iDepartmentManager.deleteById(id);
